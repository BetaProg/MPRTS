...
function aa_register(){
    if(($('#user_conf_pass').val()==$('#user_pass').val()) && ($('[name="user_type"]').val()!="") &&  ($('#user_name').val() != "")  &&   ($('#user_email').val()!="")   &&  ($('#user_mobile').val()!="") &&  ($('#user_pass').val()!="") && ($('#user_usc_no').val()!="") && ($('#user_pincode').val()!="")   ){
    ...
    $.ajax({
          ...
          success: function(value){
				 if(value!="Some Another account exists with the same credentials. Please try with some other credentials or contact administrator"){
     ...
     $.ajax({
      ...
      success: function(payment_html){
		  ...
		  if(screen.availWidth<=414){
			$('.right_content').html(payment_html);
		  }
		  else{
			$('.record_details').html(payment_html);
		  }
  ...
  function add_new_record(content_id) {
     ...
     $.ajax({
      ...
      success: function(add_content_html){
		  if(screen.availWidth<=414){
			$('.right_content').html(add_content_html);
		  }
		  else{
  ...
  function add_new_property(property_id) {
     ...
     $.ajax({
      ...
      success: function(add_property_html){
		  ...
		  if(screen.availWidth<=414){
			$('.right_content').html(add_property_html);
		  }
		  else{
			$('.record_details').html(add_property_html);
		  }
...
function add_new_owner(owner_id) {
  ...
  function add_new_expense(expense_id) {
     ...
     $.ajax({
      ...
      success: function(add_expense_html){
		  ...
		  if(screen.availWidth<=414){
			$('.right_content').html(add_expense_html);
		  }
		  else{
			$('.record_details').html(add_expense_html);
		  }
  ...
  function edit_asset(asset_id) {
     ...
     $.ajax({
      ...
      success: function(edit_asset_html){
		  ...
		  if(screen.availWidth<=414){
			$('.right_content').html(edit_asset_html);
		  }
		  else{
			$('.record_details').html(edit_asset_html);
		  }
  ...
  function edit_owner(owner_id) {
     ...
     $.ajax({
      ...
      success: function(edit_owner_html){
		  ...
		  if(screen.availWidth<=414){
			$('.details_container').html(edit_owner_html);
		  }
		  else{
			$('.record_details').html(edit_owner_html);
		  }
  ...
  function edit_expense(expense_id) {
     ...
     $.ajax({
      ...
      success: function(edit_expense_html){
		  ...
		  if(screen.availWidth<=414){
			$('.right_content').html(edit_expense_html);
		  }
		  else{
			$('.record_details').html(edit_expense_html);
		  }
  ...
  function edit_owner_resp(owner_id) {
     ...
     $.ajax({
      ...
      success: function(edit_owner_html){
          //$('.record_details').html(edit_owner_html);
		  if(screen.availWidth<=414){
			$('.right_content').html(edit_owner_html);
		  }
		  else{
			$('.record_details').html(edit_owner_html);
		  }
  ...
  function edit_tenant_resp(tenant_id) {
     ...
     $.ajax({
      ...
      success: function(edit_tenant_html){
          //$('.record_details').html(edit_tenant_html);
		  if(screen.availWidth<=414){
			$('.right_content').html(edit_tenant_html);
		  }
		  else{
			$('.record_details').html(edit_tenant_html);
		  }
  ...
  function get_all_details() {
    ...
    function edit_notification(notification_id) {
	  ...
	  //$('.right_content').html("<img src='images/preloader.gif'/>");
     $.ajax({
      url: "edit_notification.php",
      data: {
        notification_id: notification_id
      },
      type: 'post',
      cache: false,
      success: function(edit_notification_html){
          //$('.record_details').html(edit_tenant_html);
		  if(screen.availWidth<=414){
			$('.right_content').html(edit_notification_html);
		  }
		  else{
			$('.record_details').html(edit_notification_html);
		  }
      }
    })
  }